...
namespace Blob_Color_Multi_ROI
{
    public class BlobColor
    {
        ...
        public List<int> OutputRowList
        {
            get { return _outputRowList; }
        }
        public List<int> OutputColumnList
        {
            get { return _outputColumnList; }
        }
        ...
        public bool Train()
        {
            ...
            try
            {
                ...
                _hueTolerance = Tuple.Create(hMean - (int)Hmin[0], hMean, (int)Hmax[0] - hMean);
                _saturationTolerance = Tuple.Create(sMean - (int)Smin[0], sMean, (int)Smax[0] - sMean);
                _intensityTolerance = Tuple.Create(vMean - (int)Vmin[0], vMean, (int)Vmax[0] - vMean);
                _blueTolerance = Tuple.Create(bMean - (int)Bmin[0], bMean, (int)Bmax[0] - bMean);
                _greenTolerance = Tuple.Create(gMean - (int)Gmin[0], gMean, (int)Gmax[0] - gMean);
                _redTolerance = Tuple.Create(rMean - (int)Rmin[0], rMean, (int)Rmax[0] - rMean);
        ...
        public bool Run()
        {
            ...
            if (_requireNumberOfBlobs == null || _colorSpace == null)
            {
                _errMessage = "Run: Thiếu thuộc tính đầu vào";
                return false;
            }
            ...
            try
            {
                ...
                using (Mat imgInRange = new Mat())
                {
                    ScalarArray lower = GetLowerBound();
                    ScalarArray upper = GetUpperBound();

                    if (_inSetOrigin != null && _inSetOrigin.Count > 0)
                    {
                        for(int i = 0; i < _inSetOrigin.Count; i++)
                        {
                            Rectangle recCrop = CalculateBoundingRectangle(_inSetOrigin[i]);
                            using (Image<Bgr, byte> warp = _inputImage.GetSubRect(recCrop))
                            using (Image<Gray, byte> mask = CreateMask(warp.Size, _inSetOrigin[i], recCrop))
                            {
                                if (_colorSpace == RTCConst.cBlobColorTool.ColorSpace_BGR)
                                {
                                    CvInvoke.InRange(warp, lower, upper, imgInRange);
                                }
                                else
                                {
                                    Image<Bgr, byte> imgTmp = new Image<Bgr, byte>(_inputImage.Width, _inputImage.Height);
                                    CvInvoke.CvtColor(warp, imgTmp, ColorConversion.Bgr2HsvFull);
                                    CvInvoke.InRange(imgTmp, lower, upper, imgInRange);
                                    imgTmp.Dispose();
                                }

                                CvInvoke.Multiply(imgInRange, mask, mask);
                                ProcessContours(mask.Mat, recCrop);
                            }
                        }
                    }
                    else
                    {
                        ...
                        //CvInvoke.InRange(_inputImage, lower, upper, imgInRange);
                        ProcessContours(imgInRange, new Rectangle(0, 0, _inputImage.Width, _inputImage.Height));
                    }

                    // Kiểm tra pass/fail
                    _numberOfBlobsFound = _outputBlobList.Count;
                    _passed = _RequiredPass
                        ? _numberOfBlobsFound >= _requireNumberOfBlobs.Item1 && _numberOfBlobsFound <= _requireNumberOfBlobs.Item2
                        : true;
        ...
        private Rectangle CalculateBoundingRectangle(PointF[] origin)
        {
            PointF[] points = new PointF[4];
            for (int i = 0; i < 4; i++)
                points[i] = ConvertCoordinatesToOrigin(_toolOrigin, origin[i]);

            Point[] p = points.Select(pt => new Point((int)Math.Floor(pt.X), (int)Math.Floor(pt.Y))).ToArray();
            return CvInvoke.BoundingRectangle(p);
        }
        private Image<Gray, byte> CreateMask(Size size, PointF[] origin, Rectangle recCrop)
        {
            using (Image<Gray, byte> blackImage = new Image<Gray, byte>(size.Width, size.Height, new Gray(0)))
            {
                VectorOfVectorOfPoint lcontour = new VectorOfVectorOfPoint();
                VectorOfPoint contour = new VectorOfPoint();
                for (int i = 0; i < 4; i++)
                {
                    Point p = new Point((int)Math.Floor(origin[i].X - recCrop.X), (int)Math.Floor(origin[i].Y - recCrop.Y));
                    contour.Push(new Point[] { p });
                }
                lcontour.Push(contour);
                CvInvoke.DrawContours(blackImage, lcontour, -1, new MCvScalar(1), -1);
                return blackImage.Copy();
            }
        }
        private void ProcessContours(Mat image, Rectangle offsetRect)
        {
            using (VectorOfVectorOfPoint contours = new VectorOfVectorOfPoint())
            using (Mat hier = new Mat())
            {
                CvInvoke.FindContours(image, contours, hier, RetrType.Ccomp, ChainApproxMethod.ChainApproxSimple);
                    ...
                    while (k != -1)
                    {
                        ...
                        if (!_fillHoles)
                        {
                            ...
                            if (child != -1)
                            {
                                ...
                                area -= CvInvoke.ContourArea(contours[child]);
                                int childNext = (int)arr.GetValue(0, child, 0);
                                while (childNext != -1)
                                {
                                    tmp.Push(contours[childNext]);
                                    area -= CvInvoke.ContourArea(contours[childNext]);
                                    childNext = (int)arr.GetValue(0, childNext, 0);
                                }
                            }
                        }

                        Rectangle rect = CvInvoke.BoundingRectangle(contours[k]);
