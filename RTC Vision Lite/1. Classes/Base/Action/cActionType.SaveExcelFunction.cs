...
namespace RTC_Vision_Lite.Classes
{
    ...
    {
        private int _excelOrder = 0;
        private string _excelCurrentFileName = string.Empty;
        public void Run_SaveExcel_Reset()
        {
        ...
        public void Run_SaveExcel_Test()
        {
        ...
        private string Run_SaveExcel_RebuildFileNameWhenOutOfLineCount(string oldFileName)
        {
            int count = 1;
        ...
        internal void Run_SaveExcel_Prepare()
        {
            ...
            if (sFileName != _excelCurrentFileName)
            {
                if (!File.Exists(sFileName))
                    _excelOrder = 0;
                else
                {
                    _excelOrder = File.ReadLines(sFileName).Count();
                    if (_excelOrder == 1048576) // Đã đạt max dòng
                        sFileName = Run_SaveCsv_RebuildFileNameWhenOutOfLineCount(sFileName);
                }
                _excelCurrentFileName = sFileName;
            }
            ...
            try
            {
                ...
                else
                {
                    ...
                    if (!string.IsNullOrEmpty(OutputFileName.rtcValue))
                    {
                        ...
                        if (isRowToCol)
                        {
                            ...
                            for (int i = 0; i < orderColumns.Count; i++)
                            {
                                ...
                                if (orderColumns[i].DataType == cDataTypes.AutoNumber)
                                    orderColumns[i].AutoNumber = _excelOrder;
                        ...
                        Passed.rtcValue = RTCLibs.CsvFile.Write(finalRecords, OutputFileName.rtcValue, ref _excelOrder,
                            out string errMessage);

                        ErrMessage.rtcValue = new List<string>() { errMessage };
                        if (_excelOrder > cMaxRowCount.Csv) // Đã đạt max dòng
                        {
