...
namespace NsIOControllerSDK.VC3000X
{
    class CIOControllerSDK3000X
    {
        ////////////////////////////////////常量定义///////////////////////////
        // 错误码定义
        // 正确码定义
        ...
        private List<string> _listComName = new List<string>()
        {
            "COM1", "COM2", "COM3", "COM4", "COM5", "COM6", "COM7", "COM8", "COM9", "COM10", "COM11", "COM12", "COM13",
            "COM14", "COM15", "COM16", "COM17", "COM18", "COM19", "COM20"
        };

        private int _comIndex = -1;
        private IntPtr _handle = IntPtr.Zero;
        private MV_IO_SERIAL _serial = new MV_IO_SERIAL();
        /// <summary>
        ...
        public static bool IsInitVcx3000 { get; set; } = false;

        public CIOControllerSDK3000X()
        {
            IsConnected = false;
            FirmwareVersion = string.Empty;
        }
        public CIOControllerSDK3000X(string comName)
        {
            ComName = comName;
        ...
        private int RefreshFirmwareVersion()
        {
            try
            {
                int nRet = MV_OK;
                bool bGet = false;
                MV_IO_VERSION stVersion = new MV_IO_VERSION();
                int nTimeS = Environment.TickCount;
                do
                {
                    nRet = MV_IO_GetFirmwareVersion_CS(_handle, ref stVersion);
                    if (MV_OK == nRet)
                    {
                        bGet = true;
                        break;
                ...
                } while (Environment.TickCount - nTimeS < 500);

                if (bGet)
                {
                    FirmwareVersion = stVersion.nMainVersion + "." + stVersion.nSubVersion + "." + stVersion.nModifyVersion
        ...
        public static int DeInitInitVcx3000()
        {
            if (!IsInitVcx3000)
                return 0;

            int nRet = MV_IO_WinIO_DeInit_CS();
            IsInitVcx3000 = nRet != 0;
            return nRet;
        }
        public static int InitVcx3000()
        {
            if (IsInitVcx3000)
                return 0;

            int nRet = MV_IO_WinIO_Init_CS();
            if (nRet != MV_OK)
                MV_IO_WinIO_DeInit_CS();
            else
                IsInitVcx3000 = true;
        ...
        public bool Connect()
        {
            try
            {
                ...
                int nRet = InitVcx3000();
                if (nRet != MV_OK)
                {
                    ErrorMessage = "MV_IO_WinIO_Init_CS failed.";
                    DeInitInitVcx3000();
                    return false;
                }

                _comIndex = _listComName.IndexOf(ComName);
                if (_comIndex < 0)
                {
                    ErrorMessage = $"COM with name '{ComName}' is not exists.\nPlease check again.";
                    return false;
                }

                if (IntPtr.Zero == m_hDeviceHandleList[_comIndex])
                {
                    _handle = new IntPtr();

                    nRet = MV_IO_CreateHandle_CS(ref _handle);
                    if (MV_OK != nRet || IntPtr.Zero == _handle)
                    {
                        ErrorMessage = $"Creating the handle COM with name '{ComName}' failed.\nError code: {Convert.ToString(nRet, 16)}.";
                        return false;
                    }

                    _serial = new MV_IO_SERIAL();
                    _serial.strComName = ComName;

                    nRet = MV_IO_Open_CS(_handle, ref _serial);
                    if (MV_OK != nRet)
                    {
                        ErrorMessage = $"The serial port with name '{ComName}' is occupied.";
                        Disconnect();
                        return false;
                    }
                ...
                {
                    _handle = m_hDeviceHandleList[_comIndex];
                    //ErrorMessage = $"Error. The serial port is open.";
                    IsConnected = true;
                    return true;
                }
            }
        ...
        public bool Disconnect()
        {
            try
            {
                ...
                {
                    MV_IO_Close_CS(_handle);
                    MV_IO_DestroyHandle_CS(_handle);
                    if (_comIndex >= 0 && _comIndex < m_hDeviceHandleList.Length)
                        m_hDeviceHandleList[_comIndex] = IntPtr.Zero;
                    _handle = IntPtr.Zero;
                }
        ...
        private bool SetPolarity(string polarity)
        {
            try
            {
                ...
                int nType = (int)MV_IO_PNP_TYPE.MV_IO_ENABLE_PNP;
                if (Polarity == "NPN")
                    nType = (int)MV_IO_PNP_TYPE.MV_IO_ENABLE_NPN;
                int nRet = MV_IO_ExcutePNPEnable_CS(_handle, nType);
                if (MV_OK != nRet)
                {
                    ErrorMessage = "Sending the command of enabling the polarity failed. Error code:" + Convert.ToString(nRet, 16);
                    return false;
                }
                return true;
            }
        ...
        /// <summary>
        /// Đọc tín hiệu các chân input
        /// </summary>
        /// <param name="portIndexes">Danh sách các cổng input cần đọc</param>
        /// <param name="success">Cờ báo kết quả. True - Thành công; False: Thất bại</param>
        /// <returns>Mảng tín hiệu đọc được</returns>
        public int[] Read(int[] portIndexes, out bool success)
        {
            success = false;
            try
            {
                ...
                if (!IsConnected)
                {
                    ErrorMessage = "VisionBox is not connected.\nPlease check again.";
                    return new int[] { };
                }

                MV_IO_INPUT_LEVEL inputLevel = new MV_IO_INPUT_LEVEL
                {
                    nPortNumber = 0x0F
                };
                int nRet = MV_IO_GetInputLevel_CS(_handle, ref inputLevel);
                if (MV_OK != nRet)
                {
                    ErrorMessage = "Getting the electrical level status failed.";
                    return new int[] { };
                }
                ...
                result = portIndexes.Where(index => index >= 0 && index < result.Length).Select(index => result[index]).ToArray();
        ...
        private ushort[] _outputPortValues = new ushort[]
        {
            (ushort) MV_IO_PORT_NUMBER.MV_IO_PORT_1,
            (ushort) MV_IO_PORT_NUMBER.MV_IO_PORT_2,
            (ushort) MV_IO_PORT_NUMBER.MV_IO_PORT_3,
            (ushort) MV_IO_PORT_NUMBER.MV_IO_PORT_4,
            (ushort) MV_IO_PORT_NUMBER.MV_IO_PORT_5,
            (ushort) MV_IO_PORT_NUMBER.MV_IO_PORT_6,
            (ushort) MV_IO_PORT_NUMBER.MV_IO_PORT_7,
            (ushort) MV_IO_PORT_NUMBER.MV_IO_PORT_8,
            (ushort) MV_IO_PORT_NUMBER.MV_IO_PORT_9,
            (ushort) MV_IO_PORT_NUMBER.MV_IO_PORT_10,
            (ushort) MV_IO_PORT_NUMBER.MV_IO_PORT_11,
            (ushort) MV_IO_PORT_NUMBER.MV_IO_PORT_12,
            (ushort) MV_IO_PORT_NUMBER.MV_IO_PORT_13,
            (ushort) MV_IO_PORT_NUMBER.MV_IO_PORT_14,
            (ushort) MV_IO_PORT_NUMBER.MV_IO_PORT_15,
            (ushort) MV_IO_PORT_NUMBER.MV_IO_PORT_16
        };

        /// <summary>
        /// Ghi tín hiệu vào các chân out
        /// </summary>
        /// <param name="portIndexes">Danh sách các cổng input cần đọc</param>
        /// <param name="portValues">Giá trị cần ghi xuống</param>
        /// <param name="polarity"></param>
        /// <returns>True: Thành công; False: Thất bại</returns>
        public bool Write(int[] portIndexes, int[] portValues, string polarity)
        {
            try
            {
                ErrorMessage = string.Empty;
                ...
                {
                    ErrorMessage = "List port indexes can't empty.\nPlease check again.";
                    return false;
                }

                if (portValues.Length <= 0)
                {
                    ErrorMessage = "List port values can't empty.\nPlease check again.";
                    return false;
                }
                ...
                ushort outputEnablePort = 0;
                ushort outputDisablePort = 0;
                bool haveEnable = false;
                bool haveDisable = false;
                var outputEnablePorts = portIndexes.Where(index => index >= 0 && index < _outputPortValues.Length).Select(index => _outputPortValues[index]).ToArray();

                for (int i = 0; i < outputEnablePorts.Length; i++)
                {
                    if (portValues.Length == 1)
                    {
                        if (portValues[0] != 0)
                        {
                            outputEnablePort |= outputEnablePorts[i];
                            haveEnable = true;
                        }
                        else
                        {
                            outputDisablePort |= outputEnablePorts[i];
                            haveDisable = true;
                        }
                    }
                    else if (portValues.Length > i)
                    {
                        if (portValues[i] != 0)
                        {
                            outputEnablePort |= outputEnablePorts[i];
                ...
                if (haveEnable)
                {
                    MV_IO_OUTPUT_ENABLE stParam = new MV_IO_OUTPUT_ENABLE();
                    stParam.nType = (Byte)MV_IO_ENABLE_TYPE.MV_IO_ENABLE_START;
                    stParam.nPortNumber = outputEnablePort;
                    nRet = MV_IO_SetOutputEnable_CS(_handle, ref stParam);
                }
                if (haveDisable && nRet == MV_OK)
                {
                    MV_IO_OUTPUT_ENABLE stParam = new MV_IO_OUTPUT_ENABLE();
                    stParam.nType = (Byte)MV_IO_ENABLE_TYPE.MV_IO_ENABLE_END;
                    stParam.nPortNumber = outputDisablePort;
                    nRet = MV_IO_SetOutputEnable_CS(_handle, ref stParam);
                }

                if (MV_OK != nRet)
                {
                    ErrorMessage = "Sending the command of enabling output failed. Error code:" + Convert.ToString(nRet, 16);
                    return false;
                }
