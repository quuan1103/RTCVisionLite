...
namespace OriginTool
{
    public class Origin
    {
        ...
        public PointF[] InSetOriginROI1
        {
            set { _inSetOriginROI1 = value; }
        }
        public PointF[] InSetOriginROI2
        {
            set { _inSetOriginROI2 = value; }
        }
        ...
        private PointF[] _inSetOriginROI1 = null;
        private PointF[] _inSetOriginROI2 = null;
        ...
        private bool SetROI()
        {
            ...
            try
            {
                _inSetOriginROI1 = new PointF[4];
                _inSetOriginROI2 = new PointF[4];
                if (_ROI1 == null || _ROI1.Width == 0 || _ROI1.Height == 0)
                {
                    _inSetOriginROI1 = null;
                }
                else
                {
                    ...
                    for (int i = 0; i < 4; i++)
                    {
                        _inSetOriginROI1[i] = ConvertCoordinatesToNewOrigin(_toolOrigin, inPointROI[i]);
                    }
                }
                if (_ROI2 == null || _ROI2.Width == 0 || _ROI2.Height == 0)
                {
                    _inSetOriginROI2 = null;
                }
                else
                {
                    //Hàm GetRectangleVertices() lấy tọa độ 4 đỉnh của rectangle
                    ...
                    for (int i = 0; i < 4; i++)
                    {
                        _inSetOriginROI2[i] = ConvertCoordinatesToNewOrigin(_toolOrigin, inPointROI[i]);
                    }
                }
            }
            catch (Exception ex)
        ...
        public bool Run()
        {
            ...
            try
            {
                if (_originType == RTCConst.cOriginTool.OriginType_TwoLine)
                {
                    ...
                    try
                    {
                        FindLine(_inputImage, _inSetOriginROI1, _ROI1, _toolOrigin, _edgeTransitionROI1, _edgeTypeROI1, _edgeDetectionThresholdROI1,
                    _outlierDistanceThresholdROI1, _samplingPercentROI1, out lineSegment1);

                        FindLine(_inputImage, _inSetOriginROI2, _ROI2, _toolOrigin, _edgeTransitionROI2, _edgeTypeROI2, _edgeDetectionThresholdROI2,
                    ...
                    else
                    {
                        ...
                        if (lineSegment1 != null && lineSegment2 != null)
                        {
                            ...
                            if (_isShowImageResult)
                            {
                                ...
                                _outputImageShow = outputImg.ToBitmap();
                            }
